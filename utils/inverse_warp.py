...
def cam2pixel2(cam_coords, proj_c2p_rot, proj_c2p_tr, padding_mode):
    ...
    if proj_c2p_tr is not None:
        pcoords = pcoords + proj_c2p_tr  # [B, 3, H*W] #flow 2 flow+origin #
    X = pcoords[:, 0]
    Y = pcoords[:, 1]
    Z = pcoords[:, 2].clamp(min=1e-3)
...
def inverse_warp2(img, depth, ref_depth, pose, intrinsics, padding_mode='zeros'):
    ...
    src_pixel_coords, computed_depth = cam2pixel2(cam_coords, rot, tr, padding_mode)  # [B,H,W,2]#比原著多了个computed_depth
    #云图XY           云图Z
